                ...
                >
                    <div className="relative w-full h-32">
                        {albaList.map((alba, index) => {
                            ...
                            const isPrev = index === ((currentIndex - 1 + albaList.length) % albaList.length);
                            ...
                            return (
                                <div
                                    ...
                                    style={{
                                        transform: `translate(${isActive ? '-50%, 0'
                                            : isPrev ? 'calc(-125% - 2rem), 0'
                                                : 'calc(25% + 2rem), 0'
                                        ...
                                        <div className="w-32 h-32 rounded-xl overflow-hidden shadow-sm relative">
                                            ...
                                            <div className={`absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex flex-col justify-end p-3
                                                ...
                                                <p className="text-white/80 text-xs text-center">
                                                    시급 {alba.hourlyWage.toLocaleString()}원
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="absolute bottom-0 left-0 right-0 flex justify-center gap-1">
                        {albaList.map((_, index) => (
                            <div
                                key={index}
                                className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-blue-500 w-3' : 'bg-gray-300'
                                    }`}
                            />
                        ))}
                    </div>
