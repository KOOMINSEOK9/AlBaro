...
import { CalendarRange, CalendarOff, CalendarClock, CalendarDays, Plus, Pencil, X, ChevronDown, Clock, ChevronUp, ListFilter, Trash2, ChevronLeft } from 'lucide-react';
...
export default function WorkManagement() {
        ...
        <div className="group flex items-center gap-3 p-3 bg-gray-50 hover:bg-white rounded-lg transition-all duration-200 hover:shadow-md">
            <div className="flex items-center gap-3">
                <div className="relative">
                    <div className={`absolute inset-0 ${type === 'missed' ? 'bg-red-400' : 'bg-blue-400'} rounded-full opacity-20 animate-[ping_1.5s_ease-in-out_infinite]`} />
                    <div className={`relative w-2 h-2 rounded-full ${type === 'missed' ? 'bg-red-500' : 'bg-blue-500'}`} />
                </div>
                <div className="flex-1 min-w-0">
                    <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-1 text-xs text-gray-500 mt-1">
                            ...
                            <span>
                                {format(new Date(item.date), 'M월 d일(eee)', { locale: ko })} {formatTime(item.time.start)} ~ {formatTime(item.time.end)}
                            </span>
                        </div>
                    </div>
                </div>
    ...
    const DateChip = ({ slot }) => (
        <div className="group relative">
            <div className="flex items-center gap-2 bg-blue-50 hover:bg-blue-100 
                ...
                {/* 호버 시 나타나는 버튼들 */}
                <div className="absolute right-0 top-0 h-full opacity-0 
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
                    ...
                    <div className="h-full overflow-hidden">
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-[15px] font-medium text-gray-800">근무 취소</h3>
                            </div>
                    ...
                    <div className="h-full overflow-hidden">
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="text-[15px] font-medium text-gray-800">근무 추가</h3>
                            </div>
                            <span className="text-sm text-gray-500">
                    ...
                    <div className="h-full overflow-hidden relative">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <h3 className="text-[15px] font-medium text-gray-800">근무 가능 일정</h3>
                        </div>
                            ...
                            <div className="bg-gray-50 rounded-lg p-3" style={{ height: '300px' }}>
                                <div className="grid grid-cols-7 gap-1">
                                    {['월', '화', '수', '목', '금', '토', '일'].map(day => (
                                        <div key={day} className="flex items-center justify-center text-center text-xs font-medium text-gray-500 py-1" style={{ height: '40px', width: '40px' }}>
                                            {day}
                                        </div>
                                    ))}
                                    {days.map((day, index) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`
                                                    ...
                                                    ${hasTimeSlot ? 'border-2 border-green-500 bg-green-100 text-green-700' : ''}
                                                    ${isToday ? 'border-2 border-gray-500 bg-gray-100 text-gray-700' : ''}
                        ...
                        {showListModal && (
                            <>
                                ...
                                <div className={`absolute inset-0 z-20 
                                              ${isClosing ? 'animate-slideOverPanelClose' : 'animate-slideOverPanel'}`}>
                                    ...
                                    <div className="flex-1">
                                        ...
                                        >
                                            ...
                                            {Array.from({ length: 48 }).map((_, i) => {
                                                ...
                                                const isPastTime = new Date(`${format(selectedDate, 'yyyy-MM-dd')}T${timeStr}`) < new Date();

                                                return (
                                                    <option key={timeStr} value={timeStr} disabled={isPastTime}>
                                                        {formatTime(timeStr)}
                                                    </option>
                                                );
                                            })}
