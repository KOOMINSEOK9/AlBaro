...
import { Bell, ChevronRight, ChevronLeft, Clock, MapPin, X } from 'lucide-react';
      ...
      <div className={`h-full transition-transform duration-300 ease-out ${selectedNotice || showForm ? '-translate-x-full' : 'translate-x-0'}`}>
        <div className="p-4 bg-white border-b shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <button
              className="inline-flex items-center justify-center px-4 py-2 bg-blue-500 text-white text-sm font-medium rounded-lg hover:bg-blue-600 transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
        ...
        <div className="space-y-3 p-3">
          {notices.map((notice) => (
            <div
              key={notice.id}
              className="group relative p-4 cursor-pointer transition-all duration-200 bg-white hover:bg-blue-50 rounded-xl border border-gray-100 hover:border-blue-200 hover:shadow-lg transform hover:-translate-y-0.5"
              onClick={() => openNotice(notice)}
            >
              <div className="flex flex-col">
                <div className="flex justify-between items-start mb-3">
                  <h3 className={`text-base font-medium truncate pr-4 ${readNotices.has(notice.id) ? 'text-gray-500' : 'text-gray-900'}`}>
                    {notice.title}
                  </h3>
                ...
                <div className="flex items-center gap-4 text-xs text-gray-500 mb-2">
                  <div className="flex items-center gap-1">
                    ...
                    <span>{notice.time || notice.date}</span>
                  </div>
                  {notice.location && (
                    <div className="flex items-center gap-1">
